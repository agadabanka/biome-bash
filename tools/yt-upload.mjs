...
async function accessToken() {
  // seed saved creds from the environment when provided (managed runners)
  if (!fs.existsSync(CREDS) && process.env.YT_REFRESH_TOKEN) {
    fs.writeFileSync(CREDS, JSON.stringify({ refresh_token: process.env.YT_REFRESH_TOKEN }, null, 2));
  }
